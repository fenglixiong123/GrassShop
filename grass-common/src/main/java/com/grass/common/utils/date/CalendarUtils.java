...
public class CalendarUtils {
    ...
    public static String getWeekDayName(int weekday){
        if(weekday==Calendar.SUNDAY) return "周日";
        if(weekday==Calendar.MONDAY) return "星期一";
        if(weekday==Calendar.TUESDAY) return "星期二";
        if(weekday==Calendar.WEDNESDAY) return "星期三";
        if(weekday==Calendar.THURSDAY) return "星期四";
        if(weekday==Calendar.FRIDAY) return "星期五";
        if(weekday==Calendar.SATURDAY) return "星期六";
        return "-";
    }


    public static String getWeek(Date src){
        String[] weekDays = {"周日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"};
        Calendar cld = Calendar.getInstance();
        cld.setTime(src);
