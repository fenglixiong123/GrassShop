...
public class FriendlyDateUtils {
    ...
    public static Date geneDateByDateitem(int dateitem){
        if (dateitem != DATEITEM_ALL) {
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(new Date());//设置当前时间
            if (dateitem != DATEITEM_THISYEAR) {//!=6
                if (dateitem == DATEITEM_WEEK) {//==1
                    calendar.add(Calendar.DATE, -7);
                } else if (dateitem == DATEITEM_MONTH) {//==2
                    calendar.add(Calendar.MONTH, -1);
                } else if (dateitem == DATEITEM_QUARTER) {//==3
                    calendar.add(Calendar.MONTH, -3);
                } else if (dateitem == DATEITEM_HALFYERR) {//==4
                    calendar.add(Calendar.MONTH, -6);
                } else if (dateitem == DATEITEM_YEAR) {//==5
...
//                            long m = 5*(diffMinutes / 5);
                            sb.append(diffMinutes + "分钟前");
                        }
                    }
                    else if(diffHours<4){
                        long remainMin = diffMinutes - diffHours * 60;
                        if(remainMin>45) {
                            sb.append(diffHours + "小时45分钟前");
                        }else if(remainMin>30) {
                            sb.append(diffHours + "小时30分钟前");
                        }else if(remainMin>15) {
                            sb.append(diffHours + "小时15分钟前");
                        }else{
                            sb.append(diffHours + "小时前");
                        }
                    }
                    else{
//                        long remainMin = diffMinutes - diffHours * 60;
...
//                    sb.append(" " + HHmm);
    ...
    public static void main(String[] args) {
        Date date = geneDateByDateitem(DATEITEM_THISYEAR);
